...
def get_stem_ohaeng(stem: str) -> str:
    try:
        return STEM_TO_OHAENG[stem]
    except KeyError:
        raise ValueError(f"올바르지 않은 천간입니다: {stem}")


def get_branch_ohaeng(branch: str) -> str:
    try:
        return BRANCH_TO_OHAENG[branch]
    except KeyError:
        raise ValueError(f"올바르지 않은 지지입니다: {branch}")
    

def calculate_ohaeng_distribution(pillars) -> dict[str, int]:
    """
    네 기둥의 천간과 지지를 모두 세어 오행 분포를 계산한다.

    Args:
        pillars: stem과 branch 속성을 가진 기둥 목록
    ...
    for pillar in pillars:
        ...
        distribution[stem_ohaeng] += 1
        distribution[branch_ohaeng] += 1
